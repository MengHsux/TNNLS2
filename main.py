...
if __name__ == "__main__":
    ...
    BufferList = [buffer, buffer1, buffer2, buffer3, buffer4, buffer5, buffer6, buffer7, buffer7, buffer8, buffer9,
                  buffer10]
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if t > 0 and (t + 1) % args.cluster_num == 0:
            ...
            mini_batch_kmeans = MiniBatchKMeans(n_clusters=args.km_num, n_init=20, random_state=0)
            mini_batch_kmeans.fit(temp_buffer)
            labels = mini_batch_kmeans.predict(temp_buffer)
            labels = np.array(labels)
            ...
            for i in range(len(temp_sample)):
                BufferList[k].add(temp_sample[i][0], temp_sample[i][1], temp_sample[i][2], temp_sample[i][3],
                                  1 - temp_sample[i][4])

        # Train agent after collecting sufficient data
        if t >= args.batch_size and t < args.cluster_num:
            policy.train(BufferList[0], args.batch_size, kinds_number, 0, action_dim)

        if t >= args.cluster_num:
            i = int(t // args.cluster_num)
            j = int(t % (2 * i + 8))
            if j < i:
                policy.train(BufferList[j], args.batch_size, kinds_number[j], 1, action_dim)
            else:
                policy.train(BufferList[i], args.batch_size, kinds_number, 0, action_dim)
