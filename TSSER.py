...
class TD3(object):
    def __init__(
        ...
        self.actor1_optimizer = torch.optim.Adam(self.actor1.parameters(), lr=1e-3)

        self.actor2 = Actor(state_dim, action_dim, max_action).to(device)
        self.actor2_target = copy.deepcopy(self.actor2)
        self.actor2_optimizer = torch.optim.Adam(self.actor2.parameters(), lr=1e-3)
    ...
    def train(self, replay_buffer, batch_size, temp_number, judge, action_dim):
        self.train_one_q_and_pi(replay_buffer, True, batch_size, temp_number, judge, action_dim)
        self.train_one_q_and_pi(replay_buffer, False, batch_size, temp_number, judge, action_dim)
    ...
    def train_one_q_and_pi(self, replay_buffer, update_q1, batch_size, temp_number, judge, action_dim):
        self.total_it += 1
        ...
        if judge == 0:
            ...
            if update_q1:
                state, action, next_state, reward, not_done = replay_buffer.sample1(batch_size, temp_number,
                                                                                    self.actor1, action_dim)
            else:
                state, action, next_state, reward, not_done = replay_buffer.sample1(batch_size, temp_number,
                                                                                    self.actor2, action_dim)
